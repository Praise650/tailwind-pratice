...
function Navbar() {
    ...
    return (
        <div className="bg-[#4F1DA0] text-white h-screen w-1/5 px-2 border-r-black shadow-md max-lg:hidden">
            <div >
                <AppLogo />
                ...
                <div className="flex flex-col justify-between items-center gap-3 mt-10">
                    {routes.map((item, index) => {
                        return <Link to={item.path} className="text-black w-[100%] py-3 mb-3 text-start uppercase">
                            <div >
                                {item.title}
                            </div>
                        </Link>
